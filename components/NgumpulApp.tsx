...
export const CreateEvent = ({ onSaved, language, initialEvent, teamId }: { onSaved: (event: NgumpulEvent) => void, language: 'en' | 'id', initialEvent?: NgumpulEvent, teamId?: string }) => {
  ...
  const [startDate, setStartDate] = useState(() => {
    const dates = initialEvent?.dates;
...
export const EventPage = ({ event, currentUser, language, onUpdateEvent }: { event: NgumpulEvent, currentUser: UserProfile | null, language: 'en' | 'id', onUpdateEvent: (e: NgumpulEvent) => void }) => {
  const { addToast, language: ctxLanguage, guestId } = useAppContext();
  ...
  React.useEffect(() => {
    ...
    if (effectiveId) {
      const existing = (event.participants || []).find(p => p.id === effectiveId);
      if (existing && (existing.availability || []).length > 0) {
        setMyAvailability(existing.availability);
      }
    }
  }, [currentUser?.id, guestId, event.participants]); // Added event.participants to deps to ensure fresh load
  ...
  const onMouseUp = () => {
    if (isDragging) {
      // Small timeout to ensure state is flushed before saving
      setTimeout(handleSaveSchedule, 50);
    }
    setIsDragging(false);
    setDragAction(null);
  };
  ...
  }, [isDragging, myAvailability, name]); // Re-bind when deps change for the closure
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-100 dark:border-zinc-800 p-6 shadow-sm">
            ...
            >
              <motion.div variants={{ hidden: { y: 10, opacity: 0 }, show: { y: 0, opacity: 1 } }} className="flex items-center gap-3 text-zinc-600 dark:text-zinc-400 text-sm">
                ...
                <span>
                  {(() => {
                    const d1 = parseISO((event.dates || [])[0] || new Date().toISOString());
                    const d2 = parseISO((event.dates || [])[(event.dates || []).length - 1] || new Date().toISOString());
                    if (isNaN(d1.getTime()) || isNaN(d2.getTime())) return '-';
                    return `${format(d1, 'dd MMM')} - ${format(d2, 'dd MMM yyyy')}`;
                  })()}
                </span>
              </motion.div>
              {event.location && (
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-100 dark:border-zinc-800 p-6 shadow-sm">
              ...
              <div className="space-y-3">
                {bestSlots.map(([slotId, count], i) => {
                  ...
                  const isConfirmed = event.confirmedSlot === slotId;
                  return (
                    <div key={i} className={cn(
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="text-xs font-bold text-violet-900 dark:text-violet-300">
                            {(() => {
                              const d = parseISO(date);
                              return isNaN(d.getTime()) ? 'Invalid Date' : format(d, 'EEEE, dd MMM');
                            })()}
                          </div>
                          <div className="text-sm font-medium text-violet-700 dark:text-violet-400">{time}</div>
                        </div>
                        <div className="text-xs font-bold bg-violet-600 dark:bg-violet-500 text-white px-2 py-1 rounded-lg">
...
<div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-100 dark:border-zinc-800 p-4 sm:p-6 shadow-sm overflow-x-auto">
          ...
          <div className="flex flex-col sm:flex-row justify-end items-center gap-3 sm:gap-4">
            <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-400 uppercase tracking-widest bg-zinc-50 dark:bg-zinc-800/50 px-3 py-1.5 rounded-lg border border-zinc-100 dark:border-zinc-700">
              <Check size={12} className="text-emerald-500" />
              {language === 'id' ? 'Tersimpan otomatis' : 'Changes saved automatically'}
            </div>
            <button className="w-full sm:w-auto px-6 py-2 rounded-xl font-bold text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-all text-xs">
              {t.reset}
            </button>
          </div>
        </div>
  ...
  return (
    <section className="pt-32 pb-20 px-4 sm:px-6 max-w-5xl mx-auto">
      ...
      {allEvents.length === 0 ? (
        ...
        >
          {allEvents.map((event) => {
            const isPast = new Date((event.dates || [])[(event.dates || []).length - 1] || new Date()) < new Date(new Date().setHours(0, 0, 0, 0));
                  ...
                  <div className="flex items-center gap-2 text-zinc-500 dark:text-zinc-400 text-xs mt-2">
                    ...
                    <div className="flex items-center">
                      ...
                      {(event.participants || []).length > 4 ? (
                        <span className="text-xs font-medium text-indigo-600 dark:text-indigo-400">+{(event.participants || []).length - 4}</span>
                      ) : (event.participants || []).length === 0 ? (
                        <span className="text-xs text-zinc-400 dark:text-zinc-500">{language === 'id' ? 'Belum ada yang isi' : 'No one has filled yet'}</span>
                      ) : null}
                    </div>
                  </div>
                </div>
