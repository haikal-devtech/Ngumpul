...
export default function ChatPage() {
  ...
  const session = firebaseUser ? { user: firebaseUser } : null;
  const status = authLoading ? 'loading' : (firebaseUser ? 'authenticated' : 'unauthenticated');
  ...
  const sendMessage = useCallback(async (overrideType?: 'text' | 'image' | 'sticker' | 'emote' | 'poll' | 'location', overrideContent?: string, mediaUrl?: string, optimisticId?: string) => {
    ...
    const tempId = optimisticId || `temp-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    if (!optimisticId) {
  ...
  }, [session, currentUser, selectedRoom, language, addToast]);
  ...
  // ── Firestore Realtime: per-room messages listener ─────────────────────────
  useEffect(() => {
    if (!selectedRoom) return;

    const messagesCol = collection(db, "chatRooms", selectedRoom.id, "messages");
    const q = query(messagesCol, where("isDeleted", "==", false), orderBy("createdAt", "desc"), limit(50));
      ...
      snapshot.docChanges().forEach((change) => {
        if (change.type === "added") {
          ...
          setMessages((prev) => {
            ...
            // Messages are ordered desc, so we need to put them in the right place or sort
