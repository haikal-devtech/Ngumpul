...
function decodeTeamFromHash(hash: string): { id: string; name: string; desc: string; code: string } | null {
  ...
  try {
    ...
    // Attempt standard modern decoding first
    try {
      ...
      // Logic fallback for older links or legacy encoding
    ...
    console.error("Failed to decode team hash:", err);
    return null;
  }
}
...
function JoinTeamContent() {
  ...
  useEffect(() => {
    ...
    // Read team data from search param (priority) or URL hash (fallback)
    let hash = searchParams.get('code') || searchParams.get('hash') || window.location.hash.slice(1);
    
    // Fallback: Check if the hash was accidentally double-encoded/in search
    if (!hash && typeof window !== 'undefined') {
      const sp = new URLSearchParams(window.location.search);
      hash = sp.get('hash') || sp.get('code') || "";
    }
